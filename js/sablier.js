(() => {
  ...
  let startBtn, pauseBtn, resumeBtn, resetBtn;
  let bound = false;
  ...
  function onFinish() {
    stopInterval();
    hintEl.textContent = "Temps écoulé !";
    ...
    setTimeout(() => document.body.classList.remove("is-flashing"), 1600);
  }
  ...
  function start() {
    if (remainingSeconds <= 0) {
      hintEl.textContent = "Choisissez une durée pour commencer.";
      return;
    }
    unlockAudio();
    ringProgress.classList.remove("is-flash");
